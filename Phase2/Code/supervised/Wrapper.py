...
def data_generation():
    count = 0
    folder_img = os.path.join("Data_imgg")
    if os.path.exists(folder_img):
        shutil.rmtree(folder_img)
        os.mkdir(folder_img)
    else:
        os.mkdir(folder_img)
    folder_a = os.path.join("Data_ag")
    ...
    folder_h = os.path.join("H_4pointg")
    if os.path.exists(folder_h):
    ...
    for i in range(1, 5000):
        ...
        h4_filename = folder_h + "/" + str(count) + ".csv"
